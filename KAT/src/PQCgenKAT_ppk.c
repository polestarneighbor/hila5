...
main(int argc, char *argv[])
{
    ...
    int                 ret_val;
    char                *passwordC = argv[1], *passwordS = argv[2];

    // Create the REQUEST file
    sprintf(fn_req, "PQCppkKAT_%d.req", CRYPTO_SECRETKEYBYTES);
    if ( (fp_req = fopen(fn_req, "w")) == NULL ) {
        printf("Couldn't open <%s> for write\n", fn_req);
        return KAT_FILE_OPEN_ERROR;
    }
    sprintf(fn_rsp, "PQCppkKAT_%d.rsp", CRYPTO_SECRETKEYBYTES);
    ...
    do {
        ...
        if ( (ret_val = crypto_ppk_keypair(pk, sk, passwordC)) != 0) {
        ...
        if ( (ret_val = crypto_ppk_enc(ct, ss, pk, passwordS)) != 0) {
        ...
        if ( (ret_val = crypto_ppk_dec(ss1, ct, pk, sk)) != 0) {
